...
typedef VectorTree<AlignmentCell> AlignmentRow;
...
class ColumnSchemaAggregator {
 public:
    ...
    for (auto& node : VectorTreePreOrderTraversal(columns_)) {
      if (node.Parent()) {
        ...
        auto it = syntax_to_columns_map_.emplace_hint(
            syntax_to_columns_map_.end(), node.Value().path, SyntaxTreePath{});
        node.Path(it->second);
      }
      if (!node.is_leaf()) {
        // Sort subcolumns
  ...
  const std::map<SyntaxTreePath, SyntaxTreePath>& SyntaxToColumnsMap() const {
 ...
 private:
  ...
  // 1:1 map between SyntaxTreePath and column tree path.
  std::map<SyntaxTreePath, SyntaxTreePath> syntax_to_columns_map_;
};
...
static void FillAlignmentRow(
    ...
    const std::map<SyntaxTreePath, SyntaxTreePath>& columns_map,
    AlignmentRow* row) {
...
static AlignedFormattingColumnSchema ComputeColumnWidths(
  ...
  for (auto& cell : VectorTreePostOrderTraversal(column_configs)) {
    if (!cell.is_leaf()) {
      ...
      cell.Value().width = std::max(cell.Value().width, children_width) -
                           cell.Value().left_border;
    }
  }
